...
- This version depends on the global data manipulation script in `sites/nue/@shared/data/data-transforms.js`. We need to ensure that the script remains backwards compatible with this version.
...
- Copied all files we depend in the site's `@shared` folder on to the local `ui/@global` folder.
  - CSS
  - Font files
  - Layout modules and components
  - Also, the fonts file paths in the local `global.css` were updated to reference the local font files.